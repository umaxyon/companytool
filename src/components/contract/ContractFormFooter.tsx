...
import { deleteData } from './module/ContractDataModule';
import { preUpdate } from './module/ContractFormModule';


class ContractFormFooter extends React.Component<any, any> {
    ...
    public doDelete() {
        const dataId = this.props.data.contract_id;
...
const mapDispatchToProps = (dispatch: any) => ({
    ...
    doUpdate: (callback: any) => {
        dispatch(preUpdate(
            (data: any, errList: any) => {
                if (errList.length > 0) {
                    dispatch(notifyInputError(errList));
                } else if (data) {
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ContractFormFooter));
